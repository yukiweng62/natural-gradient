...
from model import TransformerARModel, TransformerConfig
...
model = TransformerARModel(model_config)

# check normalization
model = model.to(device)
if n <= 12:
    all_configs = gen_all_binary_vectors(n).to(device)
...
with_vmap = benchmark.Timer(
    stmt="ft_compute_sample_grad(params, samples)",
    setup="from __main__ import ft_compute_sample_grad",
    globals={"params": params, "samples": samples},
)
